...
module.exports.findAll = (req, res) => {
  return db.promise().query('SELECT * FROM offers')
}
/**
...
module.exports.findById = (req, res) => {
  return db.promise().query('SELECT * FROM offers WHERE id=?', [req.params.id]).then(result => {
    if (!result[0].length) return res.status(404).json({message: "Annonce introuvable, veuillez réesayer"})
    return result
  })
}
/**
...
module.exports.create = (req, res) => {
  ...
  if (!name && !content && !image && !expireAt && !startPrice) return res.status(400).json({message: 'Tout les champs sont requis'})
  const userId = res.locals.user.id
  return db.promise().query('INSERT INTO offers SET name=?, content=?, image=?, expireAt=?, startPrice=?, userId=?, createdAt=?, updatedAt=?',
    [name, content, image || req.files[0].filename, expireAt, startPrice, userId, new Date(), new Date()])
}
/**
