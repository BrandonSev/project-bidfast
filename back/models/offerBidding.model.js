...
module.exports.create = async (req, res) => {
  ...
  return db
    .promise()
    .query("SELECT o.id FROM offers AS o WHERE o.id=?", [offerId])
    .then((result) => {
      if (result[0].length <= 0)
        return res.status(404).json({ message: "Offre introuvable" });
      return db
        .promise()
        .query("INSERT INTO offer_bidding SET price=?, userId=?, offerId=?", [
          price,
          res.locals.user.id,
          offerId,
        ])
...
module.exports.findById = async (req, res) => {
  ...
  return db
    .promise()
    .query("SELECT * FROM offer_bidding WHERE id=?", [id])
    .then((result) => {
