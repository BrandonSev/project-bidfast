...
const ProfileAvatar = () => {
  ...
  const handleFileUpload = async (e) => {
    ...
    await axios
      .put(`${process.env.REACT_APP_API_URL}/api/users/${userId}`, formData, {
  ...
  return (
    <>
      <form
        className={`avatar_profile__container ${hover ? "hover" : ""}`}
        ...
        {loading ? (
          <div className="loader">
            <ClockLoader
              ...
              height={4}
              css={
                "display: flex;justify-content: center;position: absolute;left: 50%;top: 50%;transform:translate(-50%,-50%)"
              }
            />
          </div>
        ) : (
